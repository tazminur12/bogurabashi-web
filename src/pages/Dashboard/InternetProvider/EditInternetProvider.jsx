...
import { FiWifi, FiMapPin, FiPhone, FiMail, FiGlobe, FiDollarSign, FiUsers, FiClock, FiCalendar, FiStar, FiArrowLeft, FiSave } from 'react-icons/fi';

const EditInternetProvider = () => {
  ...
  useEffect(() => {
    const fetchProvider = async () => {
      try {
        setFetching(true);
        const res = await axiosSecure.get(`/internet-providers/${id}`);
        if (res.data.success) {
          setFormData(res.data.data);
        } else {
          Swal.fire('ত্রুটি', 'প্রোভাইডারের তথ্য পাওয়া যায়নি', 'error');
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axiosSecure.put(`/internet-providers/${id}`, formData);
      if (res.data.success) {
        Swal.fire({
          title: 'সফল!',
          text: 'ইন্টারনেট প্রোভাইডারের তথ্য আপডেট করা হয়েছে',
          icon: 'success',
      ...
      Swal.fire('ত্রুটি', 'তথ্য আপডেট করা যায়নি', 'error');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-800">ইন্টারনেট প্রোভাইডার এডিট করুন</h1>
                <p className="text-gray-600 mt-1">প্রোভাইডারের তথ্য আপডেট করুন</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-gray-200">
              <button
                type="submit"
                disabled={loading}
                className="flex-1 bg-gradient-to-r from-green-600 to-emerald-600 text-white py-3 px-6 rounded-lg hover:from-green-700 hover:to-emerald-700 transition-all duration-300 font-semibold disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
...
export default EditInternetProvider; 