...
import { FaCar, FaSave, FaArrowLeft, FaPhone, FaMapMarkerAlt, FaMoneyBillWave, FaInfoCircle, FaCalendarAlt, FaUser, FaShieldAlt, FaEdit } from "react-icons/fa";
...
const UpdateRentCar = () => {
  ...
  const { data: car, isLoading, error } = useQuery({
    queryKey: ['rent-car', id],
    queryFn: async () => {
      const res = await axiosSecure.get(`/rent-cars/${id}`);
      return res.data;
    },
    enabled: !!id
  });
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axiosSecure.put(`/rent-cars/${id}`, formData);
      
      if (response.data.modifiedCount > 0) {
        Swal.fire({
          title: "✅ সফল!",
          text: "গাড়ির তথ্য সফলভাবে আপডেট করা হয়েছে",
          icon: "success",
          confirmButtonText: "ঠিক আছে"
        }).then(() => {
          navigate("/dashboard/rent-car-admin");
        });
      }
    } catch (error) {
      console.error("Error updating car:", error);
      Swal.fire("❌ ত্রুটি", "গাড়ির তথ্য আপডেট করতে সমস্যা হয়েছে", "error");
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-[#f8f9fa] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-4">গাড়ির তথ্য লোড করতে সমস্যা হয়েছে</p>
          <Link
    ...
    <div className="min-h-screen bg-[#f8f9fa] py-10">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-3xl font-bold text-[#333] mb-1">✏️ গাড়ির তথ্য সম্পাদনা করুন</h1>
            <p className="text-sm text-gray-600">গাড়ির তথ্য আপডেট করুন</p>
          </div>
          <Link
            to="/dashboard/rent-car-admin"
        ...
        {car && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="font-medium text-blue-800">বর্তমান গাড়ির তথ্য</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm">
              <div><span className="font-medium">ব্র্যান্ড:</span> {car.brand}</div>
              <div><span className="font-medium">মডেল:</span> {car.model}</div>
              <div><span className="font-medium">ধরন:</span> {car.type}</div>
              <div><span className="font-medium">ভাড়া:</span> ৳{formatPrice(car.rentPerDay)}/দিন</div>
              <div><span className="font-medium">অবস্থান:</span> {car.location}</div>
              <div><span className="font-medium">অবস্থা:</span> {car.status}</div>
            </div>
          </div>
        )}
