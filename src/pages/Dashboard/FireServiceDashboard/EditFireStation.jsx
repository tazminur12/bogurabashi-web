...
import {
  FiZap, FiMapPin, FiPhone, FiUser, FiSave,
  FiArrowLeft, FiInfo, FiShield, FiEdit
} from "react-icons/fi";

const EditFireStation = () => {
  ...
  useEffect(() => {
    const fetchFireStation = async () => {
      try {
        setIsFetching(true);
        const res = await axiosSecure.get(`/fire-stations/${id}`);
        ...
        let errorMessage = 'ফায়ার স্টেশন তথ্য লোড করা যায়নি';
        if (error.response?.status === 404) {
          errorMessage = 'ফায়ার স্টেশন পাওয়া যায়নি';
        } else if (error.response?.status === 500) {
          errorMessage = 'সার্ভার ত্রুটি হয়েছে';
        }
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formData.name.trim()) newErrors.name = "স্টেশনের নাম প্রয়োজন";
    if (!formData.officer.trim()) newErrors.officer = "অফিসারের নাম প্রয়োজন";
    ...
    if (!formData.type) newErrors.type = "স্টেশনের ধরন নির্বাচন করুন";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axiosSecure.put(`/fire-stations/${id}`, stationData);

      if (res.data.message === 'Updated successfully') {
        Swal.fire({
          title: "সফল!",
          text: "ফায়ার স্টেশন সফলভাবে আপডেট করা হয়েছে",
          icon: "success",
      ...
      let errorMessage = 'ফায়ার স্টেশন আপডেট করা যায়নি';
      if (error.response?.status === 404) {
        errorMessage = 'ফায়ার স্টেশন পাওয়া যায়নি';
      } else if (error.response?.status === 500) {
        errorMessage = 'সার্ভার ত্রুটি হয়েছে';
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-orange-50 to-yellow-50 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-bold text-gray-800 mb-2">ফায়ার স্টেশন সম্পাদনা করুন</h1>
          <p className="text-gray-600 text-lg">ফায়ার স্টেশনের তথ্য আপডেট করুন</p>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-6">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="flex-1 inline-flex items-center justify-center px-6 py-3 bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-700 hover:to-red-700 text-white font-semibold rounded-lg shadow-lg transition-all duration-200 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isLoading ? (
                    <>
